...
def chat_loop(
    ...
    conv = None
    conv = reset_use_template(conv)
    for i, entry in enumerate(json_data):
        if 'complex' not in entry:
            if i % 2 == 0:
                conv = None
                conv = reset_use_template(conv)
            ...
            if i % 1000 == 999:
                print("data nums: ", i)
                # 保存更新后的 JSON 数据
                save_json_path = "/media/sdb/liutao/datasets/s2c/prompts/large_data/unique_prompts/simple_prompts_10/s2c_vote_10000.json"
